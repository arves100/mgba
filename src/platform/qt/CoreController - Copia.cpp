...
CoreController::CoreController(mCore* core, QObject* parent)
...
#ifdef M_CORE_GBA
		case PLATFORM_GBA:
			context->core->setPeripheral(context->core, mPERIPH_GBA_LUMINANCE,
			                             controller->m_inputController->luminance());
			break;
#endif
		default:
			break;
	...
	m_threadContext.resetCallback = [](mCoreThread* context) {
		...
		context->core->setVideoBuffer(context->core, reinterpret_cast<color_t*>(controller->m_activeBuffer.data()),
		                              controller->screenDimensions().width());
	...
	m_threadContext.logger.d.log = [](mLogger* logger, int category, enum mLogLevel level, const char* format,
	                                  va_list args) {
			...
			if (strncmp(loadstateMessage, format, strlen(loadstateMessage)) == 0) {
				...
				if (slot == 0) {
					format = "Loaded suspend state";
			           ...
			           strncmp(savestateMessage, format, strlen(savestateMessage)) == 0) {
				va_list argc;
				va_copy(argc, args);
				int slot = va_arg(argc, int);
				va_end(argc);
		...
		message = QString().vsprintf(format, args);
		QMetaObject::invokeMethod(controller, "logPosted", Q_ARG(int, level), Q_ARG(int, category),
...
QImage CoreController::getPixels() {
	...
	QImage image(reinterpret_cast<const uchar*>(buffer.constData()), size.width(), size.height(), stride,
...
void CoreController::start() {
	...
	m_threadContext.core->setVideoBuffer(m_threadContext.core, reinterpret_cast<color_t*>(m_activeBuffer.data()),
	                                     size.width());
...
void CoreController::setPaused(bool paused) {
	...
	if (paused) {
		addFrameAction([this]() { mCoreThreadPauseFromThread(&m_threadContext); });
	} else {
...
void CoreController::screenshot() {
	mCoreThreadRunFunction(&m_threadContext, [](mCoreThread* context) { mCoreTakeScreenshot(context->core); });
}
#endif
...
static GBASIODriver adapterGBABeta;

#include <mgba/internal/gba/io.h>

static struct mTimingEvent AdapterGBABetaTiming;

static uint8_t stepMode = 0;
static bool isSending = false;
static uint8_t lastCommand = 0;
static uint16_t checc = 0;
static uint8_t dataa[256];
...
void _MAGBEvent(struct mTiming* timing, void* user, uint32_t cyclesLate) {
	struct GBASIODriver* gate = (GBASIODriver*) user;

	if (gate->p->mode != SIO_NORMAL_8) {
		return;
	}

//	gate->p->p->memory.io[REG_SIODATA8] = data;
	gate->p->p->memory.io[REG_SIOCNT] &= 8;
	gate->p->siocnt = GBASIONormalClearStart(gate->p->siocnt);
	if (GBASIONormalIsIrq(gate->p->siocnt)) {
		GBARaiseIRQ(gate->p->p, IRQ_SIO, cyclesLate);
	}
}

uint16_t _MAGBReg(GBASIODriver* driver, uint32_t address, uint16_t value) {
	switch (address) {
	case REG_SIOCNT:
		if (value & 0x80) {
			int32_t cycles = GBA_ARM7TDMI_FREQUENCY / 524288;
			mTimingDeschedule(&driver->p->p->timing, &AdapterGBABetaTiming);
			mTimingSchedule(&driver->p->p->timing, &AdapterGBABetaTiming, cycles);
			break;
		}
		break;
	case REG_SIODATA8: {
		if (!isSending) {
			if (stepMode == 3) { // command id
				lastCommand = value;
			} else if (stepMode == 6) { // len
				datalen = value;
				checc = 0;
			} else if (stepMode == 7) {
				if ((datalen - checc) > 0) {
...
void CoreController::attachMobileAdapter() {
	if (platform() == PLATFORM_GB) {
		// TODO: Sincronizzazione con https://github.com/mid-kid/mgba
		return;
	}

	// adapterGBABeta.init = _MAGBInit;
	// adapterGBABeta.deinit = _MAGBDeinit;
	// adapterGBABeta.load = _MAGBLoad;
	// adapterGBABeta.unload = _MAGBUnload;
	adapterGBABeta.writeRegister = _MAGBReg;

	AdapterGBABetaTiming.context = &adapterGBABeta;
	AdapterGBABetaTiming.callback = _MAGBEvent;
	AdapterGBABetaTiming.priority = 0x80;

	Interrupter interrupter(this);
	clearMultiplayerController();
	m_threadContext.core->setPeripheral(m_threadContext.core, mPERIPH_GBA_MOBILE_ADAPTER, &adapterGBABeta);
}
